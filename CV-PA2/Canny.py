...
def non_maximum_suppression(G, theta):
    ...
    thetadict = {
        0   : (0, 1, 0, -1),    # East, West
        45  : (1, 1, -1, -1),   # SouthEast, NorthWest 
        90  : (-1, 0, 1, 0),    # South, North
        135 : (1, -1, -1, 1),   # SouthWest, NorthEast
    }
    for i in range(1, H-1):
        for j in range(1, W-1):
            i_offset1, j_offset1, i_offset2, j_offset2 = thetadict[theta[i, j] % 180]
            if  G[i + i_offset1, j + j_offset1] <= G[i,j] >= G[i + i_offset2, j + j_offset2]:
                out[i,j] = G[i,j]
    ######################################
    #        END OF YOUR CODE            #
    ######################################
